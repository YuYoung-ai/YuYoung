...
try{
  ...
  ck('영업 담당자별 건수는 기본 접힘',!await salesChip('영업가').isVisible()&&await page.getAttribute('#hstSalesCountsToggle','aria-expanded')==='false');
  await page.selectOption('#hstSales','영업가');
  ck('접힌 상태에서도 드롭다운 필터는 동작하고 자동으로 펼쳐지지 않음',
    await page.locator('#hstTableHost tbody tr').count()===1&&!await page.locator('#hstSalesCountsList').isVisible());
  await page.selectOption('#hstSales','all');
  await page.locator('#hstSalesCountsToggle').focus();await page.keyboard.press('Enter');
  ck('키보드로 영업담당자 건수 펼치기',await salesChip('영업가').isVisible()&&await page.getAttribute('#hstSalesCountsToggle','aria-expanded')==='true');
  await salesChip('영업가').click();
  ck('영업담당자 칩은 병원·VOC 최신 선택 1건 표시',await page.locator('#hstTableHost tbody tr').count()===1);
  ck('칩 필터 적용 후에도 펼침 유지',await page.locator('#hstSalesCountsList').isVisible());
  ck('다른 담당자·미지정 칩 유지',await salesChip('영업나').count()===1&&await salesChip('__missing__').count()===1);
  await salesChip('영업가').click();
  ck('같은 칩 재클릭 해제',await page.inputValue('#hstSales')==='all'&&await page.locator('#hstTableHost tbody tr').count()===4);
  ...
  ck('필터 초기화는 사용자의 펼침 상태 유지',await page.locator('#hstSalesCountsList').isVisible());
  await page.click('#hstSalesCountsToggle');
  ck('다시 누르면 접히고 필터·처리 건수는 유지',!await page.locator('#hstSalesCountsList').isVisible()&&await page.locator('#hstTableHost tbody tr').count()===4);
  await page.click('#hstSalesCountsToggle');
  ...
  ck('DB 갱신으로 팝업을 다시 그려도 펼침 상태 유지',await page.locator('#hstSalesCountsList').isVisible());
