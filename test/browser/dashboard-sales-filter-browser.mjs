...
try{
  ...
  ck('접힌 상태에서도 드롭다운 필터는 동작하고 자동으로 펼쳐지지 않음',
    await page.locator('#hstTableHost tbody tr').count()===2&&!await page.locator('#hstSalesCountsList').isVisible());
  ...
  ck('다시 누르면 접히고 필터·처리 건수는 유지',!await page.locator('#hstSalesCountsList').isVisible()&&await page.locator('#hstTableHost tbody tr').count()===5);
  await page.click('#hstSalesCountsToggle');
