...
## v3.8.0 — 모바일 갤러리 선택 · 사진 3장 처리 지연 완화

- `handover`의 ② 증상·③ 해결 후 사진에 **바로 촬영 / 갤러리 선택**을 분리했다.
  갤러리 입력에는 `capture`를 두지 않아 휴대폰에 저장된 사진을 고를 수 있다.
- 세 사진은 선택 즉시 압축·사전 업로드하며, 전체 동시 업로드를 2개로 제한해
  모바일 회선과 GAS 실행 슬롯을 한꺼번에 점유하지 않는다. 최종 시트 기록 요청에는
  Base64를 다시 보내지 않고 사진 ID 참조만 보낸다.
- 서버는 사진 한 장마다 `현장 사진` 시트 11개 열 전체를 읽지 않고, 기록 ID 1열에서
  해당 기록 범위를 찾은 뒤 필요한 행만 읽는다. 최종 3장 상태 갱신도 범위 1회 쓰기로 묶었다.
- 시트 스키마 변경·마이그레이션은 없다.
...
- 응답이 커지지 않도록 유형별 사례를 `maxCases`(기본 50 · 최대 500)로 자르되,
  **처리일 최신순**(같은 날은 최근 기록 행 우선)으로 남긴다. 잘라낸 건수는 `truncated` 로 함께 주며
  화면·엑셀·보고서 텍스트 모두에 표시된다.
...
- `handoverGAS/exec?action=ping&warm=1` → `{"ver":"3.8.0","warmed":true,...}` (Core 확장 + 스프레드시트 예열 반영 확인).
  ...
  `3.7.0` 미만이면 사진 설명 문구 버튼이 뜨지 않는 구버전,
  `3.8.0` 미만이면 사진 범위 조회·3장 상태 일괄 갱신 최적화가 없는 구버전이다.
