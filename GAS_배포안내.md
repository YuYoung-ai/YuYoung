...
## v3.9.0 — 장비 S/N 사진 선택 전환 · 현장 사진 화면 삭제

- **장비 S/N 사진이 필수에서 선택으로 바뀌었다.** 사진 없이도 시트에 기록된다.
  - 서버는 사진이 실제로 왔거나, 클라이언트가 `photoRequired:true` 로 **"이 요청에 사진을 실었다"**
    고 선언한 요청만 예전처럼 사진까지 성공해야 전체 성공으로 본다
    (붙였는데 payload 에서 빠지는 사고는 계속 잡아낸다).
  - 사진 없는 기록의 응답은 `photo:{required:false, saved:false}` 다.
- **`handover.html` 의 '현장 사진(증상·해결 후)' 입력 화면을 삭제했다** — 전송 데이터 문제로
  쓰지 않기로 했다. 함께 쓰이던 **✎ 문구 관리**(자주 쓰는 문구) 버튼·모달도 화면에서 빠졌다.
  - **서버는 그대로다.** `현장 사진` 시트·`CAUSE`/`AFTER` 구분·`photo_add`·`photophrase` 는
    이미 쌓인 기록을 읽고 출력하기 위해 전부 살아 있다(label.html·asreport.html 영향 없음).
  - **시트 변경·마이그레이션 없음.**
- 배포 순서 제약: **handover_gas 를 먼저** 재배포해야 사진 없는 기록이 저장된다.
  구버전 서버에 사진 없이 저장하면 `사진이 필수인 기록인데 사진이 전송되지 않았습니다` 로 거절된다.
...
- **시트를 미리 만들 필요 없다.** 없으면 문구 버튼이 안 보일 뿐이다.
  ※ v3.9.0 에서 handover 화면의 문구 버튼·**✎ 문구 관리** 모달은 현장 사진과 함께 삭제됐다.
    서버 API(`photophrase`·`photophrase_save`)는 그대로 남아 있다.
...
- `handoverGAS/exec?action=ping&warm=1` → `{"ver":"3.9.0","warmed":true,...}` (Core 확장 + 스프레드시트 예열 반영 확인).
  ...
  `3.8.0` 미만이면 사진 범위 조회·3장 상태 일괄 갱신 최적화가 없는 구버전,
  `3.9.0` 미만이면 **사진 없는 기록을 거절하는** 구버전이다(장비 S/N 사진이 아직 필수).
